...
/**
 * This class is responsible for writing relevant locations fields to CSV file.
 * 
 * @author ah
 *
 */
public class LocationsFileWriter {
	 ...
	 * 
	 * @param fileName
	 *            the file name of the csv file (including the extension)
	 * @param locations
	 *            a list of the locations to be written to file
	 * @throws Exception
	...
	public static void WriteCsvFile(String fileName, List<Location> locations) throws Exception {
		...
		try {
			...
			LOGGER.severe(String.format("Error while trying to write CSV file! message: %s", e.getMessage()));
			throw e;
		} finally {
			try {
				fileWriter.flush();
				...
				LOGGER.severe(String.format("Error while trying to flush CSV file! message: %s", e.getMessage()));
